<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true" row-key="id">
      ...
      <el-table-column label="父分类ID" align="center" prop="parentId" min-width="100" />
      ...
      <el-table-column label="操作" align="center" min-width="110px" fixed="right">
        <template #default="scope">
          <el-button link type="primary" @click="openForm('update', scope.row.id)" v-hasPermi="['restaurant:category:update']">编辑</el-button>
          <el-button link type="danger" @click="handleDelete(scope.row.id)" v-hasPermi="['restaurant:category:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
defineOptions({ name: 'RestaurantCategory' })
